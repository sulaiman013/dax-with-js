 ...
 * Drives preview-holo.html in Edge/Chrome over the DevTools protocol with no
...
const PORT = 9337;
const PAGE = 'file:///' + path.resolve(__dirname, 'preview-holo.html').replace(/\\/g, '/');
const PROFILE = fs.mkdtempSync(path.join(os.tmpdir(), 'tph-holo-'));
  ...
  try {
    ...
    let st = await evalJs(`(function(){
      ...
      return {
        ...
        beams: n('.tph-node .beam'), caps: n('.tph-node .cap'), plane: n('.tph-plane'),
    ...
    if (st.beams !== 12) throw new Error('expected 12 columns, got ' + st.beams);
    if (st.caps !== 12) throw new Error('expected 12 column caps, got ' + st.caps);
    if (st.plane !== 1) throw new Error('expected one holo plane, got ' + st.plane);
    ...
    if (shot.result && shot.result.data) {
      const out = path.resolve(__dirname, 'holo-smoke.png');
