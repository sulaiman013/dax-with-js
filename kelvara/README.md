...
![The map view](screenshot-page.png)

*The map view: the figure, the explanation pane and a short guide. Everything
else moved a gesture away.*
...
| [`screenshot-rail.png`](screenshot-rail.png) | The filter rail, open. |
| [`screenshot-detail.png`](screenshot-detail.png) | The breakdown view. |
...
## Two views, one visual, and why there is no real drill-through

Native drill-through needs the visual to build selection identities and hand one
to the host. HTML Content never does, so a custom visual cannot tell Power BI
which body region you right-clicked, cannot pass a filter to another page, and
cannot navigate to one at all.

Rather than pretend otherwise, the page carries both views itself:

- **Map view.** The figure, the explanation pane, the guide. Left-click a region
...
so a visible control opens it unscoped. That also solves the discoverability
problem, because right-click is invisible.
